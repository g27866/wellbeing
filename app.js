...
// ============ 其餘常量 ============
const ENDPOINTS = {
  getReservations: uid => `${API_BASE}/reservations?uid=${uid}`, // 若未實作可移除
};
...
async function checkCustomerExist(){
  ...
  }catch(e){ console.error(e); showBinding(); }
}
...
bindBtn.onclick=async()=>{
  ...
  try{
    const res=await api("POST", CUSTOMER_BIND_URL, {
